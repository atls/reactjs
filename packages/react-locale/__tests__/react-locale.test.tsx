...
import { LocaleProvider, LocaleConsumer, LocaleContext, LocaleStore, useLocale } from '../src/index'
...
describe('Test suit for react-locale', function describer() {
    ...
    test('should track LocaleStore changes + useLocale', function tester() {
        ...
        const ProviderComponent = () => <LocaleProvider value={ ctxStore }><Wrapper /></LocaleProvider>;

        const Wrapper = () => <div><ConsumerComponent /></div>;

        const ConsumerComponent = () => {
            ctxStore.set('ru');

            const [getCurrent, getSupported, setLocale] = useLocale();

            return <h1 className={'consumer'}>{`${getCurrent} ${getSupported}`}</h1>
        };

        expect(render(<App />).find('.consumer').text()).toBe('ru en,ru')
    })
})